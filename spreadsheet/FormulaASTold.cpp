...
// При делении на 0 выбрасывайте ошибку вычисления FormulaError
    double Evaluate(const SheetInterface& sheet) const override {
        double result = ACTION.at(type_)(lhs_->Evaluate(sheet), rhs_->Evaluate(sheet));
    ...
    std::pair<const Expr*, const Expr*> GetOperands() const
    {
        return std::pair{ lhs_.get(), rhs_.get() };
    }
...
private:
    ...
    static const std::unordered_map<Type, std::function<double(double, double)>> ACTION;
};

const std::unordered_map<BinaryOpExpr::Type, std::function<double(double, double)>> 
BinaryOpExpr::ACTION = {
            std::pair{ Type::Add,       [](double lhs, double rhs) { return lhs + rhs; } },
            std::pair{ Type::Subtract,  [](double lhs, double rhs) { return lhs - rhs; } },
            std::pair{ Type::Multiply,  [](double lhs, double rhs) { return lhs * rhs; } },
            std::pair{ Type::Divide,    [](double lhs, double rhs) { return lhs / rhs; } }
};
...
// Реализуйте метод Evaluate() для унарных операций.
    double Evaluate(const SheetInterface& sheet) const override {
        return ACTION.at(type_)(operand_->Evaluate(sheet));
    }
...
private:
    ...
    static const std::unordered_map<Type, std::function<double(double)>> ACTION;
};

const std::unordered_map<UnaryOpExpr::Type, std::function<double(double)>>
UnaryOpExpr::ACTION = {
            std::pair{ Type::UnaryPlus, [](double operand) {return operand; } },
            std::pair{ Type::UnaryMinus,[](double operand) {return - operand; } }
};
...
using CellSet = std::unordered_set<const CellInterface*, std::hash<const CellInterface*>>;

//Вспомогательная функция для конвертации позиций в адреса ячеек
inline CellSet GetAddressFromPosition(const SheetInterface& sheet, const std::vector<Position> positions)
{
    CellSet cells;
    for (const Position pos : positions)
    {
        cells.emplace(sheet.GetCell(pos));
    }
    return cells;
}
...
{
public:
    CellExpr(const SheetInterface& sheet, const Position pos)
        : sheet_(sheet), position_(pos) 
    {
        using namespace std::string_literals;
        if (IsCycleRef())
        {
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        const CellInterface::Value result = sheet_.GetCell(position_)->GetValue();
...
private:
    ...
    bool IsCycleRef()
    {
        std::unique_ptr<CellSet> visited = std::make_unique<CellSet>(CellSet()); //black colored
        std::unique_ptr<CellSet> gray_list = std::make_unique<CellSet>(CellSet()); //gray colored
        return IsCycleRefImpl(sheet_, sheet_.GetCell(position_), visited.get(), gray_list.get());
    }   

    static bool IsCycleRefImpl(const SheetInterface& sheet, const CellInterface* cell, CellSet* black, CellSet* gray)
    {
        gray->insert(cell);
        CellSet cell_list = GetAddressFromPosition(sheet, cell->GetReferencedCells());

        for (const CellInterface* cell : cell_list)
        {
            if (black->count(cell))
            {
                continue;
            }
            if (gray->count(cell) || !IsCycleRefImpl(sheet, cell, black, gray))
            {
                return false;
            }
        }
        black->emplace(make_move_iterator(gray->begin()), make_move_iterator(gray->end()));
        gray->clear();
        return true;
    }
};
...
std::set<Position>* GetReferencedCellsImpl(const ASTImpl::Expr* expr, std::set<Position>* result = nullptr)
{
    if (!result)
    {
        try
        {
            result = new std::set<Position>();
        }
    ...
    if (const ASTImpl::CellExpr* cell = dynamic_cast<const ASTImpl::CellExpr*>(expr))
    {
        result->emplace(cell->GetPosition());
    }
    else if (const ASTImpl::BinaryOpExpr* bexpr = dynamic_cast<const ASTImpl::BinaryOpExpr*>(expr))
    {
        auto [lhs, rhs] = bexpr->GetOperands();
        result = GetReferencedCellsImpl(lhs, result);
        result = GetReferencedCellsImpl(rhs, result);
    }
    else if (const ASTImpl::UnaryOpExpr* uexpr = dynamic_cast<const ASTImpl::UnaryOpExpr*>(expr))
    {
        result = GetReferencedCellsImpl(uexpr->GetOperand(), result);
    }
    return result;
}

std::set<Position> FormulaAST::GetReferencedCells() const
{
    std::set<Position>* result = GetReferencedCellsImpl(root_expr_.get());
    return result ? *result : std::set<Position>();
}
